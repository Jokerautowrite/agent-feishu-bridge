...
$Backend = if ($env:AGENT_BRIDGE_BACKEND) { $env:AGENT_BRIDGE_BACKEND } else { Read-Value 'Agent backend (codex/opencode/claude/chuang/openclaw/hermes/grok/gemini)' ((Read-ExistingEnv 'AGENT_BRIDGE_BACKEND') -replace '^$','codex') }
...
if ($Backend -notin @('codex','opencode','claude','chuang','openclaw','hermes','grok','gemini')) { throw "Unsupported backend: $Backend" }
